                        ...
                        <div class="col-md-12 col-xs-12">
                            ...
                            <form action="{{ route('campaings.update', $campaign->id) }}" id="startCampaignForm" class="form-horizontal" method="post" enctype="multipart/form-data" >
                                    ...
                                    <div class="col-sm-8">
                                        <select id="country_id" class="form-control select2" name="country_id">
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->id }}" {{ $campaign->country_id == $country->id ? 'selected' :'' }}>{{ $country->name }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="form-group  {{ $errors->has('state_id')? 'has-error':'' }}">
                                    ...
                                    <div class="col-sm-8">
                                        <select id="state_id" class="form-control select2" id="state_select" name="state_id">
                                            @if($previous_states->count() > 0)
                                                @foreach($previous_states as $state)
                                                    <option value="{{ $state->id }}" {{ $campaign->state_id == $state->id ? 'selected' :'' }}>{{ $state->state_name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                        <p class="text-info">
                                ...
                                <div class="form-group  {{ $errors->has('city_id')? 'has-error':'' }}">
                                    <label for="city_id" class="col-sm-4 control-label">@lang('app.city')</label>
                                    <div class="col-sm-8">
                                        <select id="city_id" class="form-control select2" id="city_select" name="city_id">
                                            @if($previous_cities->count() > 0)
                                                @foreach($previous_cities as $city)
                                                    <option value="{{ $city->id }}" {{ $campaign->city_id == $city->id ? 'selected':'' }}>{{ $city->city_name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                        <p class="text-info">
